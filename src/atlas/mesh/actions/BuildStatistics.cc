...
void quad_quality(double& eta, double& rho, const PointLonLat& p1, const PointLonLat& p2, const PointLonLat& p3,
    ...
    double max_inner = std::max(std::max(std::max(std::abs(M_PI_2 - a1), std::abs(M_PI_2 - a2)), std::abs(M_PI_2 - a3)),
                                std::abs(M_PI_2 - a4));
...
void build_statistics(Mesh& mesh) {
    ...
    if (nodes.has_field("dual_volumes")) {
        ...
        array::ArrayView<double, 1> dual_delta_sph = array::make_view<double, 1>(
            nodes.add(Field("dual_delta_sph", array::make_datatype<double>(), array::make_shape(nodes.size(), 1))));
